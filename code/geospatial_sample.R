...
dat <- read_csv("output/wettable_acres_output.csv")

counties <- read_sf("counties.shp")

counties

counties <- st_transform(counties, crs = 32019)
...
dat_spatial <- st_as_sf(dat, coords = c("lon", "lat"), crs = 32019)
